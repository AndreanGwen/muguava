...
document.querySelector(".checkout").addEventListener("click", function () {
  window.location.href =
    "https://api.whatsapp.com/send/?phone=%2B6289653540366&text&type=phone_number&app_absent=0"; // Ganti dengan URL tujuan
});
...
listkeranjangHTML.addEventListener("click", (event) => {
  let posisitionClick = event.target;
  if (
    posisitionClick.classList.contains("minus") ||
    posisitionClick.classList.contains("plus")
  ) {
...
const tambahkeranjang = (produk_id) => {
  let positionThisProductInCart = carts.findIndex(
    (value) => value.produk_id == produk_id
  );
...
const addCartToHTML = () => {
  ...
  if (carts.length > 0) {
    carts.forEach((cart) => {
      ...
      let positionproduk = listproduk.findIndex(
        (value) => value.id == cart.produk_id
      );
      ...
      newCart.innerHTML = `
                ...
                <div class="totalharga">${
                  parseFloat(info.harga) * cart.quantity
                }</div>
